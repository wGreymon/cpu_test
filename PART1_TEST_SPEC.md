...
>
> **V0.9 变更**（Intel/AMD 补测后）：区分“执行有效性”和“测量稳定性”。命令非零退出、指标解析失败或正确性校验失败仍记为 `ERROR` 并使批次不完整；CV 超参考线改记为 `UNSTABLE`，保留为平台波动信息，不再使测试层返回非零或把整批标成不完整。
...
| 标准 | 规则 |
|---|---|
| 执行有效性 | 命令正常退出、主指标成功解析且正确性校验通过；任一不满足均记为 `ERROR` |
| 测量稳定性 | 计算/内存类 CV 以 3%、系统/存储类 CV 以 5% 为参考线；超出记为 `UNSTABLE`，先排查干扰并进行一次受控补测，若仍复现则作为平台波动特征报告，不等同执行失败 |
...
MEM-00 或环境门槛失败时立即中止；普通测试组发生命令非零退出、指标解析失败、正确性校验失败或必需数据缺失时，继续收集其余结果，但总控最终返回退出码 2，整批状态为“不完整”。仅 CV 超稳定性参考线时标记 `UNSTABLE` 并正常完成批次。

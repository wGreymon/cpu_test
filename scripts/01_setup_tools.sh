...
if [[ ! -x "$SRC/byte-unixbench-5.1.3/UnixBench/Run" ]]; then
    cd "$SRC"
    wget -q https://github.com/kdlucas/byte-unixbench/archive/refs/tags/v5.1.3.tar.gz \
...
if [[ ! -d "$SRC/lmbench" ]]; then
    cd "$SRC"
    git clone -q https://github.com/intel/lmbench.git \
        && cd lmbench \
...
if [[ ! -x "$BIN/fs_mark" ]]; then
    cd "$SRC"
    git clone -q https://github.com/josefbacik/fs_mark.git \
...
if [[ ! -f "$ROOT_DIR/workloads/silesia.tar" ]]; then
    ...
    wget -q https://sun.aei.polsl.pl/~sdeor/corpus/silesia.zip \
...
if [[ ! -x "$BIN/mlc" ]]; then
    ...
    wget -q https://downloadmirror.intel.com/793041/mlc_v3.11.tgz \
